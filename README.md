...
Modifie le bouton dans le `.html` (nom, émoji, bulle), les sons dans le `.js` et la couleur (`--hue`) dans le `.css`
7. Copie aussi le test `tests/piano.test.js` en `tests/mon-instrument.test.js` et adapte-le
8. Vérifie que tout marche : `npm install` puis `npm test`
9. Commit, push, puis ouvre une **Pull Request** vers ce dépôt 🎉
